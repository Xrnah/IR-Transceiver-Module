...
std::vector<String> ssidBlacklist;  // permanently unknown
std::vector<String> ssidSkiplist;   // known but failed during this cycle

#define EEPROM_SIZE 96
#define WIFI_RETRY_DELAY 1000  // ms delay between retry cycles
#define WIFI_RETRY_LIMIT 0      // 0 = retry forever
...
// 8th Floor - School of Engineering
	...
	// * 816/818 - Hydrolic Laboratory / Soil Test Laboratory
...
bool connectToWiFi(const char* ssid, const char* password) {
  ...
  Serial.printf("🔌 Trying to connect to WiFi: %s", ssid);
  for (int i = 0; i < 20; i++) {
...
bool autoConnectWiFi() {
  ...
  for (int i = 0; i < found; i++) {
    ...
    if (!isKnown) {
      Serial.printf("⚠️  Skipping unknown SSID: %s (not in table)\n", ssidScanned.c_str());
    }
  }

  // 3. Connect to strongest known SSID
  if (bestIndex != -1) {
...
// 🔁 Retry wrapper: runs autoConnectWiFiOnce until connected
